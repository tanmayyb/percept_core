...
namespace goal_heuristic 
{
  ...
  __device__ double3 calculate_current_vec(double3 point_dist_vec_normalized, double3 goal_vec) 
  {
    // Project goal vector onto the plane perpendicular to point_dist_vec using vector rejection.
    double3 current_vec = goal_vec - (point_dist_vec_normalized * dot(point_dist_vec_normalized, goal_vec));
    ...
    double k_circ) 
  {
    ...
    if (!(dot(point_dist_vec_normalized, goal_vec_normalized) < -1e-5 && dot(point_dist_vec, point_rvel_vec) < -1e-5)) 
    {
      ...
      if (dist_to_point < detect_shell_rad && norm(point_rvel_vec) > 1e-10) 
      {
        ...
        double3 current_vec = calculate_current_vec(point_dist_vec_normalized, goal_vec);
