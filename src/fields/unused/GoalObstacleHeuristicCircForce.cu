...
__device__ double3 calculate_rotation_vector(
    ...
    obstacle_current_vec = mass_dist_vec_normalized * dot(mass_dist_vec_normalized, obstacle_vec) - obstacle_vec;  
    ...
    goal_current_vec = goal_vec - mass_dist_vec_normalized * dot(mass_dist_vec_normalized, goal_vec);

    current_vec = normalized(goal_current_vec) + normalized(obstacle_current_vec);
    if (norm(current_vec) < 1e-10){
        current_vec = make_double3(0.0, 0.0, 1.0);
    }
    current_vec = normalized(current_vec);
...
__host__  void hello_cuda_world(){
  std::cout<<"Hello CUDA World! -From Goal Obstacle Heuristic Circ Force Kernel <3"<<std::endl;
}
