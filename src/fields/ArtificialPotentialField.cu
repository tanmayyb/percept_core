...
namespace artificial_potential_field 
{
    ...
    double k_force) 
  {
    ...
    double3 mass_dist_vec = mass_position - agent_position;

    double dist_to_mass_center = norm(mass_dist_vec);
    
    double dist_to_mass = dist_to_mass_center - (agent_radius + point_radius);

    dist_to_mass = fmax(dist_to_mass, 1e-5); 

    double3 force_vec = make_double3(0.0, 0.0, 0.0);

    if (dist_to_mass <= detect_shell_rad) 
    {
      double3 mass_dist_vec_normalized = mass_dist_vec * (1.0 / dist_to_mass_center);
        
      double inv_dist = 1.0 / dist_to_mass;

      double inv_shell = 1.0 / detect_shell_rad;

      double force_mag = (inv_dist - inv_shell) * (k_force / dist_to_mass);
      
      force_vec = mass_dist_vec_normalized * (-1.0 * force_mag);
    }
